...
<c:set var="profile" value='<%=session.getAttribute("loginUser")%>' />
	...
	<div class="col-12">
		...
		<div class="col-8" style="float: left; text-align: center;">
			${ room.TUTOR_USER_user_id } 님과 대화</div>
...
<img id="profileImg" class="img-fluid"
					src="resources/uploadFiles/userProfile/${ loginUser.changeName }" style = "display:none">
<input type="text" id="nickname" value = "${ room.TUTOR_USER_user_id }" style = "display:none">
...
<script type="text/javascript">
 ...
 function connect() {
	    sock = new SockJS('http://192.168.30.197:8888/goodplace/chat');
 ...
 function send() {
  ...
  if(msg != ""){
	  ...
	  message.message_content = $("#message").val()
  	  ...
  	  message.USER_user_id = '${loginUser.email}'
  	  message.CLASS_class_id = '${room.CLASS_class_id}'
  	  message.message_sender = '${loginUser.email}'
  }
	console.log(message);
